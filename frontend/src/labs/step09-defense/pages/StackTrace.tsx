    ...
    <div>
      ...
      <ExpandableSection isOpen={results.length > 0}>
        <div className="mt-2 max-h-[300px] overflow-auto">
          {results.map((r, i) => (
            <div key={i} className="text-xs p-2 mb-1 rounded bg-error-bg-light">
              ...
              {r.stack && <pre className="text-xs mt-1 overflow-auto max-h-[100px] bg-code-bg text-code-text p-1 rounded">{r.stack}</pre>}
              {r.debug && <pre className="text-xs mt-1 overflow-auto bg-code-bg text-code-text p-1 rounded">{JSON.stringify(r.debug, null, 2)}</pre>}
