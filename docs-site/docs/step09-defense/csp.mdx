...
| [クリックジャッキング](../step07-design/clickjacking.mdx) | CSP の `frame-ancestors` ディレクティブはクリックジャッキング対策としても機能する。`X-Frame-Options` の現代的な代替 |
| [セッションハイジャック](../step04-session/session-hijacking.mdx) | CSP がない環境で XSS が成功すると Cookie の窃取によるセッションハイジャックが可能。CSP は Cookie 窃取の防止にも貢献する |
...
- [Content Security Policy - 理解度テスト](./csp-quiz)
