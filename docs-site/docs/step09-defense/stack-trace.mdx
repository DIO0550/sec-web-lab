...
| ラボ | 関連性 |
|------|--------|
| [詳細エラーメッセージ露出](./error-messages.mdx) | スタックトレースはエラーメッセージ漏洩の一形態。本ラボではスタックトレースに特化し、error-messages ラボではDB構造やSQLクエリなどより広範な情報漏洩を扱う |
...
- [スタックトレース漏洩 - 理解度テスト](./stack-trace-quiz)
