...
| ラボ | 関連性 |
|------|--------|
| [弱いパスワードポリシー](./weak-password-policy) | 弱いパスワードの登録を許可すると、ブルートフォースの成功確率が劇的に上がる |
| [デフォルト認証情報](./default-credentials) | デフォルトパスワードは辞書の最上位に含まれ、ブルートフォースで最初に突破される |
| [平文パスワード保存](./plaintext-password) | ブルートフォースで突破されたアカウントのパスワードが平文で保存されていると、他サービスへの攻撃にも直接使用される |
...
- [ブルートフォース攻撃 - 理解度テスト](./brute-force-quiz)
