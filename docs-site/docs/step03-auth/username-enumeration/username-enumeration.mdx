...
| ラボ | 関連性 |
|------|--------|
| [ブルートフォース攻撃](../brute-force/brute-force.mdx) | ユーザー名列挙で確認されたユーザー名に対してブルートフォース攻撃を実行することで、攻撃効率が飛躍的に向上する。列挙はブルートフォースの「偵察フェーズ」に位置づけられる |
| [弱いパスワードポリシー](../weak-password-policy/weak-password-policy.mdx) | ユーザー名が判明し、かつパスワードポリシーが弱い場合、少数のパスワード候補で突破される可能性が高まる |
| [デフォルト認証情報](../default-credentials/default-credentials.mdx) | ユーザー名列挙で `admin` 等のデフォルトアカウントの存在が確認された場合、デフォルトパスワードの試行で即座にログインされる |
...
[→ 実際に試す](./username-enumeration-tryit.mdx)
...
- [ユーザー名列挙 - 理解度テスト](./username-enumeration-quiz.mdx)
