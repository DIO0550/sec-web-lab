...
| **難易度** | ★★☆ 中級 |
| **前提知識** | ハッシュ関数の基本概念（一方向関数）、[平文パスワード保存](./plaintext-password)ラボの完了 |
...
| ラボ | 関連性 |
|------|--------|
| [平文パスワード保存](./plaintext-password) | 平文保存の次のステップとして「ハッシュ化すれば安全か？」を学ぶ。MD5/SHA1 では不十分であることを理解する |
| [ブルートフォース攻撃](./brute-force) | MD5 の高速性はオンラインブルートフォースだけでなく、オフラインでの総当たりも容易にする |
...
- [弱いハッシュアルゴリズム - 理解度テスト](./weak-hash-quiz)
