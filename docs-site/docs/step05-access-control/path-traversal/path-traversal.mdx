...
| ラボ | 関連性 |
|------|--------|
| [ディレクトリリスティング](../../step01-recon/directory-listing/directory-listing.mdx) | ディレクトリリスティングでファイル構造を把握した後、パストラバーサルで非公開ファイルにアクセスするという段階的な攻撃が可能 |
| [IDOR](../idor/idor.mdx) | 同じ「サーバーがクライアント入力を信頼しすぎる」パターン。IDORがリソースIDの書き換えであるのに対し、パストラバーサルはファイルパスの書き換え |
| [OSコマンドインジェクション](../../step02-injection/command-injection/command-injection.mdx) | パストラバーサルで設定ファイルを読み取り、その情報を使ってコマンドインジェクションを行うという複合攻撃が可能 |
...
[→ 実際に試す](./path-traversal-tryit.mdx)
...
- [Path Traversal - 理解度テスト](./path-traversal-quiz.mdx)
