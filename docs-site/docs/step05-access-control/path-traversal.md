---
title: "Path Traversal — `../` でサーバーの秘密ファイルを読み取る"
sidebar_position: 2
---
...
| ラボ | 関連性 |
|------|--------|
| [ディレクトリリスティング](../step01-recon/directory-listing) | ディレクトリリスティングでファイル構造を把握した後、パストラバーサルで非公開ファイルにアクセスするという段階的な攻撃が可能 |
| [IDOR](./idor.md) | 同じ「サーバーがクライアント入力を信頼しすぎる」パターン。IDORがリソースIDの書き換えであるのに対し、パストラバーサルはファイルパスの書き換え |
| [OSコマンドインジェクション](../step02-injection/command-injection) | パストラバーサルで設定ファイルを読み取り、その情報を使ってコマンドインジェクションを行うという複合攻撃が可能 |
