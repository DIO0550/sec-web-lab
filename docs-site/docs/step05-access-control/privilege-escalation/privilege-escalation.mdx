...
| ラボ | 関連性 |
|------|--------|
| [IDOR](../idor/idor.mdx) | IDOR が水平方向（同じ権限レベルの他ユーザー）の認可不備であるのに対し、権限昇格は垂直方向（上位権限）の認可不備。どちらもアクセス制御の欠如が原因 |
| [Mass Assignment](../mass-assignment/mass-assignment.mdx) | Mass Assignment で `role` フィールドを改ざんすることは、権限昇格の一手法。API経由でロールを変更し、永続的な管理者アクセスを得られる |
| [CSRF](../../step04-session/csrf/csrf.mdx) | 管理者のブラウザに CSRF 攻撃を仕掛けることで、管理者権限の操作を攻撃者が間接的に実行するという複合攻撃が可能 |
...
[→ 実際に試す](./privilege-escalation-tryit.mdx)
...
- [権限昇格 (Privilege Escalation) - 理解度テスト](./privilege-escalation-quiz.mdx)
