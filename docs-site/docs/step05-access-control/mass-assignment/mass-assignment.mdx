...
| ラボ | 関連性 |
|------|--------|
| [IDOR](../idor/idor.mdx) | IDORで他ユーザーのデータにアクセスし、Mass Assignmentで権限を変更するという複合攻撃が可能 |
| [権限昇格](../privilege-escalation/privilege-escalation.mdx) | Mass Assignmentによる `role` フィールドの改ざんは、権限昇格の一手法である |
| [パストラバーサル](../path-traversal/path-traversal.mdx) | 同じ「クライアント入力を信頼しすぎる」パターンの別の脆弱性 |
...
[→ 実際に試す](./mass-assignment-tryit.mdx)
...
- [Mass Assignment - 理解度テスト](./mass-assignment-quiz.mdx)
