...
| ラボ | 関連性 |
|------|--------|
| [IDOR](./idor.mdx) | IDORで他ユーザーのデータにアクセスし、Mass Assignmentで権限を変更するという複合攻撃が可能 |
| [権限昇格](./privilege-escalation.mdx) | Mass Assignmentによる `role` フィールドの改ざんは、権限昇格の一手法である |
| [パストラバーサル](./path-traversal.mdx) | 同じ「クライアント入力を信頼しすぎる」パターンの別の脆弱性 |
...
- [Mass Assignment - 理解度テスト](./mass-assignment-quiz)
