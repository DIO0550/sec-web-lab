...
| ラボ | 関連性 |
|------|--------|
| [XSS](../xss/xss.mdx) | XSS で `document.location` を変更すればオープンリダイレクトと同等のことが可能。オープンリダイレクトは「サーバー側のリダイレクト」版 |
| [SSRF](../../step06-server-side/ssrf/ssrf.mdx) | サーバーサイドでの URL パラメータ操作。オープンリダイレクトがクライアント向けの誘導なのに対し、SSRF はサーバーに内部リソースへのアクセスを強制する |
...
- [Open Redirect - 理解度テスト](./open-redirect-quiz.mdx)
