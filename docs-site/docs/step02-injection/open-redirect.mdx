...
| **攻撃例** | `/api/labs/open-redirect/vulnerable/redirect?url=https://evil.example.com` |
| **技術スタック** | Hono API |
| **難易度** | ★☆☆ 入門 |
...
| バックエンド | `backend/src/labs/step02-injection/open-redirect/` |
| フロントエンド | `frontend/src/labs/step02-injection/pages/OpenRedirect.tsx` |
...
| ラボ | 関連性 |
|------|--------|
| [XSS](./xss) | XSS で `document.location` を変更すればオープンリダイレクトと同等のことが可能。オープンリダイレクトは「サーバー側のリダイレクト」版 |
| [SSRF](../step06-server-side/ssrf.mdx) | サーバーサイドでの URL パラメータ操作。オープンリダイレクトがクライアント向けの誘導なのに対し、SSRF はサーバーに内部リソースへのアクセスを強制する |
...
- [Open Redirect - 理解度テスト](./open-redirect-quiz)
