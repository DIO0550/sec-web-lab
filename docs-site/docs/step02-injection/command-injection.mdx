...
| バックエンド | `backend/src/labs/step02-injection/command-injection/` |
| フロントエンド | `frontend/src/labs/step02-injection/pages/CommandInjection.tsx` |
...
| ラボ | 関連性 |
|------|--------|
| [SQL インジェクション](./sql-injection) | 同じ「入力値がコードとして解釈される」パターン。シェルの代わりに SQL が対象 |
| [XSS](./xss) | 同じインジェクション系。ブラウザ側の JavaScript コンテキストが対象 |
| [SSRF](../step06-server-side/ssrf.mdx) | サーバーサイドのリソースにアクセスする攻撃。コマンドインジェクションで curl を実行すれば SSRF と同等の攻撃が可能 |
...
- [OS Command Injection - 理解度テスト](./command-injection-quiz)
