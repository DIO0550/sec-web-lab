---
title: HTTP Parameter Pollution (HPP) — 同じパラメータの重複で検証をすり抜ける
sidebar_position: 6
---
...
| ラボ | 関連性 |
|------|--------|
| [SQL Injection](./sql-injection) | 同じ「入力値の不適切な処理」カテゴリ。SQLi は入力がコードとして解釈される問題、HPP は入力の参照先が検証と実行で異なる問題 |
| [オープンリダイレクト](./open-redirect) | HPP でリダイレクト先 URL パラメータを汚染し、オープンリダイレクトを誘発するケースがある |
