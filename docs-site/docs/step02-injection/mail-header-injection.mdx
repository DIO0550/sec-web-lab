---
title: Mail Header Injection — メールヘッダーに改行コードを注入してスパムの踏み台にする
sidebar_position: 5
---
...
| [OSコマンドインジェクション](./command-injection) | 同じインジェクション系の脆弱性。ユーザー入力がプロトコルやシステムの制御構造として解釈されるという共通パターン。コマンドインジェクションではシェルのメタ文字、メールヘッダーインジェクションでは CRLF が制御文字として機能する |
