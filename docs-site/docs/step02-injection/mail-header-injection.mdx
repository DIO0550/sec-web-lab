...
| ラボ | 関連性 |
|------|--------|
| [CRLFインジェクション](../step06-server-side/crlf-injection.mdx) | 同じ CRLF 注入の手法だが、対象が HTTP レスポンスヘッダー。メールヘッダーインジェクションはメールプロトコルに対する CRLF 注入であり、攻撃の原理（改行コードが構造の区切りとして機能する）は共通 |
...
- [Mail Header Injection - 理解度テスト](./mail-header-injection-quiz)
