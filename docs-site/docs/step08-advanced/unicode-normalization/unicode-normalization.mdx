...
| ラボ | 関連性 |
|------|--------|
| [Reflected XSS](../../step02-injection/xss/xss.mdx) | Unicode 正規化バイパスは XSS フィルタの回避手法の一つ。XSS の基本を理解した上で、フィルタ回避のテクニックとして学ぶ |
| [SSTI](../../step08-advanced/ssti/ssti.mdx) | テンプレートインジェクションでも Unicode 正規化を使ったフィルタ回避が適用される場合がある。入力サニタイズのバイパス手法として共通 |
...
[→ 実際に試す](./unicode-normalization-tryit.mdx)
...
- [Unicode Normalization Bypass - 理解度テスト](./unicode-normalization-quiz.mdx)
