...
| ラボ | 関連性 |
|------|--------|
| [SSTI](./ssti.mdx) | どちらもサーバーサイドでの任意コード実行に発展する脆弱性。SSTI はテンプレートエンジン経由、デシリアライゼーションはデータ復元処理経由 |
...
| [Mass Assignment](../step05-access-control/mass-assignment.mdx) | プロトタイプ汚染と Mass Assignment は「意図しないプロパティの設定」という共通パターン。入力データのフィルタリングが対策の要 |
...
- [Insecure Deserialization - 理解度テスト](./deserialization-quiz)
