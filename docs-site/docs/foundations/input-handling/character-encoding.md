...
1バイト文字 (ASCII互換):
┌─────────┐
│ 0xxxxxxx │   先頭ビット "0" → 1バイト文字
└─────────┘
  0x00-0x7F (0-127)

2バイト文字:
┌─────────┬─────────┐
│ 110xxxxx │ 10xxxxxx │   先頭 "110" → 2バイト文字の開始
└─────────┴─────────┘    後続 "10"  → 継続バイト
  0xC0-0xDF  0x80-0xBF

3バイト文字 (日本語等):
┌─────────┬─────────┬─────────┐
│ 1110xxxx │ 10xxxxxx │ 10xxxxxx │   先頭 "1110" → 3バイト文字の開始
└─────────┴─────────┴─────────┘
...
セキュリティ上の注意:
  先頭バイトのビットパターンでバイト数が決まる
  → 不正なバイト列（非最短形式）はデコーダが拒否すべき
  → 0xC0 0xBC は "<" の非最短形式（2バイトで表現しているが本来1バイト）
```
...
| ラボ | 関連する知識 |
|------|--------------|
| [XSS](../../step02-injection/xss) | 文字エンコーディングの問題はXSSの入口になり得る。非最短形式やエンコーディング変換による特殊文字の出現が、エスケープ処理を迂回する手段として悪用される |

### SQLインジェクション

| ラボ | 関連する知識 |
|------|--------------|
| [SQLインジェクション](../../step02-injection/sql-injection) | Shift_JISの5C問題に代表される、マルチバイト文字によるエスケープ処理の迂回がSQLインジェクションの原因になる。UTF-8統一とプレースホルダの使用が根本対策 |
