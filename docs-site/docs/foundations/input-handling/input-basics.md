...
SQLインジェクション対策の最も確実な方法は**プレースホルダ（パラメータ化クエリ）**である。
...
| ラボ | 関連する知識 |
|------|--------------|
| [XSS](../../step02-injection/xss) | HTMLエスケープの不備によるスクリプト注入 |
| [SQLインジェクション](../../step02-injection/sql-injection) | プレースホルダを使わないSQL組み立ての危険性 |
