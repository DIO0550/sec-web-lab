...
| ラボ | 関連性 |
|------|--------|
| [XSS (Stored / Reflected)](../../step02-injection/xss/xss.mdx) | Web Storage Abuse の起点となる脆弱性。XSS がなければ localStorage からのトークン窃取は発生しない。XSS 対策が最も重要な防御線 |
| [セッションハイジャック](../../step04-session/session-hijacking/session-hijacking.mdx) | Cookie にセッション ID を保存する場合でも HttpOnly 属性がなければ同様の窃取が可能。「トークンの保存場所」と「アクセス制御属性」の両方が重要 |
| [Cookie 操作](../../step04-session/cookie-manipulation/cookie-manipulation.mdx) | HttpOnly、Secure、SameSite 属性の詳細。安全なトークン管理には Cookie 属性の正しい設定が不可欠 |
| [JWT脆弱性](../../step08-advanced/jwt-vulnerabilities/jwt-vulnerabilities.mdx) | JWT 自体のアルゴリズム混同や署名検証不備の問題。トークンの保存場所が安全でも、JWT の実装に脆弱性があれば攻撃は成立する |
...
[→ 実際に試す](./web-storage-abuse-tryit.mdx)
...
- [Web Storage Abuse - 理解度テスト](./web-storage-abuse-quiz.mdx)
