...
| ラボ | 関連性 |
|------|--------|
| [CSRF](../step04-session/csrf.mdx) | CSRF も被害者に意図しないリクエストを送らせる攻撃。クリックジャッキングは「クリック」を悪用し、CSRF は「リクエスト」を直接偽造する違いがある |
| [HTTPでの機密データ送信](sensitive-data-http.mdx) | セキュリティヘッダーの未設定が脆弱性の原因となる点が共通 |
...
- [クリックジャッキング - 理解度テスト](./clickjacking-quiz)
