...
| ラボ | 関連性 |
|------|--------|
| [推測可能なパスワードリセット](password-reset.mdx) | パスワードリセットトークンが推測可能な場合、レート制限がなければトークンの総当たりによりパスワードリセットを乗っ取れる |
| [ビジネスロジックの欠陥](business-logic.mdx) | レート制限の欠如はビジネスロジック上の設計ミスの一種であり、クーポンの無制限適用等と同様に「回数制限すべき処理を制限していない」問題 |
...
- [レート制限なし - 理解度テスト](./rate-limiting-quiz)
