...
| [クリックジャッキング](clickjacking.mdx) | `X-Frame-Options` と `CSP frame-ancestors` の欠如がクリックジャッキングの直接的な原因。本ラボのセキュリティヘッダー設定がクリックジャッキング防御の根幹となる |
...
- [セキュリティヘッダー - 理解度テスト](./security-headers-quiz)
