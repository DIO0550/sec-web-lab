...
| ラボ | 関連性 |
|------|--------|
| [ビジネスロジックの欠陥](../business-logic/business-logic.mdx) | 価格改ざんはビジネスロジックの欠陥とも密接に関連する。サーバー側でのバリデーション不足が共通の根本原因 |
| [CSRF](../../step04-session/csrf/csrf.mdx) | CSRFもCookieの自動送信を悪用する攻撃。署名なしデータの信頼ではCookie値の改ざんが、CSRFではCookieの自動送信がそれぞれ問題となる |
| [Cookie操作](../../step04-session/cookie-manipulation/cookie-manipulation.mdx) | Cookieの仕組みと改ざん手法を詳しく学べるラボ。本ラボの前提知識として有用 |
...
- [署名なしデータの信頼 - 理解度テスト](./unsigned-data-quiz.mdx)
