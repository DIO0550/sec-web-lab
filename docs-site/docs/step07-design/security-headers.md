---
title: Security Headers — セキュリティレスポンスヘッダの未設定・設定ミス
sidebar_position: 8
---
...
| **技術スタック** | Hono API |
| **難易度** | ★☆☆ 入門 |
| **前提知識** | HTTPレスポンスヘッダーの基本構造、XSSの概念（[XSS](../step02-injection/xss) を先に学ぶことを推奨）、[HTTPヘッダーからの情報漏洩](../step01-recon/header-leakage) |
...
| ラボ | 関連性 |
|------|--------|
| [XSS](../step02-injection/xss) | CSP はXSS攻撃の被害を軽減する多層防御。XSS の脆弱性がコード上に存在しても、CSP が適切に設定されていればインラインスクリプトの実行や外部スクリプトの読み込みがブロックされる |
...
| [HTTPヘッダーからの情報漏洩](../step01-recon/header-leakage) | 不要なヘッダーの「除去」（header-leakage）と、セキュリティヘッダーの「付与」（本ラボ）はセットで行うべき。両方合わせてヘッダーの安全な管理になる |
| [セキュリティヘッダーの欠如](../step01-recon/header-exposure) | 偵察フェーズでセキュリティヘッダーの欠如を発見する方法を扱う。本ラボは欠如が見つかった場合の影響と対策を深く掘り下げる |
