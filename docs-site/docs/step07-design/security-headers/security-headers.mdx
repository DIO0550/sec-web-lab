...
| **技術スタック** | Hono API |
| **難易度** | ★☆☆ 入門 |
| **前提知識** | HTTPレスポンスヘッダーの基本構造、XSSの概念（[XSS](../../step02-injection/xss/xss.mdx) を先に学ぶことを推奨）、[HTTPヘッダーからの情報漏洩](../../step01-recon/header-leakage/header-leakage.mdx) |
...
| ラボ | 関連性 |
|------|--------|
| [XSS](../../step02-injection/xss/xss.mdx) | CSP はXSS攻撃の被害を軽減する多層防御。XSS の脆弱性がコード上に存在しても、CSP が適切に設定されていればインラインスクリプトの実行や外部スクリプトの読み込みがブロックされる |
| [クリックジャッキング](../clickjacking/clickjacking.mdx) | `X-Frame-Options` と `CSP frame-ancestors` の欠如がクリックジャッキングの直接的な原因。本ラボのセキュリティヘッダー設定がクリックジャッキング防御の根幹となる |
| [HTTPヘッダーからの情報漏洩](../../step01-recon/header-leakage/header-leakage.mdx) | 不要なヘッダーの「除去」（header-leakage）と、セキュリティヘッダーの「付与」（本ラボ）はセットで行うべき。両方合わせてヘッダーの安全な管理になる |
| [セキュリティヘッダーの欠如](../../step01-recon/header-exposure/header-exposure.mdx) | 偵察フェーズでセキュリティヘッダーの欠如を発見する方法を扱う。本ラボは欠如が見つかった場合の影響と対策を深く掘り下げる |
...
[→ 実際に試す](./security-headers-tryit.mdx)
...
- [セキュリティヘッダー - 理解度テスト](./security-headers-quiz.mdx)
