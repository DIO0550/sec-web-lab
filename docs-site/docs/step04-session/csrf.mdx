...
| ラボ | 関連性 |
|------|--------|
| [Cookie 操作](./cookie-manipulation) | `SameSite` 属性が未設定の Cookie は CSRF 攻撃のクロスサイトリクエストでも送信されるため、CSRF の成立条件に直結する |
| [セッションハイジャック](./session-hijacking) | CSRF がセッションの「悪用」であるのに対し、セッションハイジャックはセッションの「窃取」。どちらも Cookie ベースのセッション管理の弱点を突く |
