...
| ラボ | 関連性 |
|------|--------|
| [XSS](../../step02-injection/xss/xss.mdx) | アップロードしたHTMLファイルが同一オリジンで配信される場合、Stored XSSと同等の攻撃が可能になる |
| [パストラバーサル](../../step05-access-control/path-traversal/path-traversal.mdx) | ファイル名に `../../` を含めることで、意図しないディレクトリにファイルを書き込むパストラバーサル攻撃と組み合わせられる |
...
- [ファイルアップロードの検証不備 - 理解度テスト](./file-upload-quiz.mdx)
