...
| ラボ | 関連性 |
|------|--------|
| [XXE](../xxe/xxe.mdx) | XXEの `SYSTEM` キーワードで `http://` を指定すると、SSRFと同等の攻撃が可能。XMLパーサーを経由したSSRF |
| [ファイルアップロード](../file-upload/file-upload.mdx) | アップロードされたファイルのURL取得処理でSSRFが発生する場合がある（例: 画像URLを指定してサーバーがダウンロード） |
...
- [SSRF (Server-Side Request Forgery) - 理解度テスト](./ssrf-quiz.mdx)
