...
| ラボ | 関連性 |
|------|--------|
| [SSRF](../ssrf/ssrf.mdx) | XXEの `SYSTEM` キーワードで `http://` プロトコルを使うと、サーバーから内部ネットワークへのリクエストを送信でき、SSRFと同等の攻撃が可能 |
| [SQLインジェクション](../../step02-injection/sql-injection/sql-injection.mdx) | XXEもSQLiも「ユーザー入力がコードとして解釈される」という共通の根本原因を持つ。XXEではXMLのDTD構文、SQLiではSQL構文 |
...
- [XXE (XML External Entity) - 理解度テスト](./xxe-quiz.mdx)
