...
| **技術スタック** | Hono API |
| **難易度** | ★☆☆ 入門 |
| **前提知識** | HTTPレスポンスヘッダーの基本（[HTTPヘッダーからの情報漏洩](./header-leakage) を先に学ぶことを推奨） |
...
- [HTTPヘッダーからの情報漏洩](./header-leakage) と共通の図を参照
...
| ラボ | 関連性 |
|------|--------|
| [HTTPヘッダーからの情報漏洩](./header-leakage) | 不要なヘッダーの「除去」と、セキュリティヘッダーの「付与」はセットで行う。両方合わせてヘッダーの安全な管理になる |
| [クリックジャッキング](../step07-design/clickjacking.mdx) | `X-Frame-Options` の欠如を直接悪用する攻撃。本ラボで学んだ防御策が直接役立つ |
...
- [セキュリティヘッダーの欠如 - 理解度テスト](./header-exposure-quiz)
