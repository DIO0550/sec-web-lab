---
title: HTTP Header Information Leakage — HTTPレスポンスヘッダーからの情報漏洩
sidebar_label: 解説
sidebar_position: 1
---
...
| フロントエンド | `docs-site/src/components/labs/step01/HeaderLeakageLab.tsx` |
...
| ラボ | 関連性 |
|------|--------|
| [不要なヘッダー露出](../header-exposure/header-exposure.mdx) | 同じHTTPヘッダーの問題を異なる観点（セキュリティヘッダーの欠如）で扱う |
| [エラーメッセージからの情報漏洩](../error-message-leakage/error-message-leakage.mdx) | エラーレスポンスのボディからも技術情報が漏洩する。ヘッダー漏洩と組み合わせると偵察精度が上がる |
...
[→ 実際に試す](./header-leakage-tryit.mdx)
...
- [HTTPヘッダーからの情報漏洩 - 理解度テスト](./header-leakage-quiz.mdx)
