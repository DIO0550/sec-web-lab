...
import styles from './RaceConditionLab.module.css';
    ...
    <div>
      ...
      <ExpandableSection isOpen={results.length > 0}>
        <div className={styles.resultList}>
          {results.map((r, i) => (
            <div key={i} className={`${styles.resultItem} ${r.success ? styles.resultSuccess : styles.resultError}`}>
...
export function RaceConditionLab() {
  ...
  const purchase = async (mode: 'vulnerable' | 'secure'): Promise<PurchaseResult> => {
  ...
  const handlePurchase = async (mode: 'vulnerable' | 'secure') => {
    ...
    try {
      ...
      const err: PurchaseResult = { success: false, message: (e as Error).message };
      if (mode === 'vulnerable') {
  ...
  const handleRace = async (mode: 'vulnerable' | 'secure') => {
    ...
    const promises = Array.from({ length: 5 }, () =>
      purchase(mode).catch((e) => ({ success: false, message: (e as Error).message }) as PurchaseResult)
    );
