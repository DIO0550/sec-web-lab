...
6. **glow filter禁止（矢印）**: 矢印の `<path>` 要素に `filter="url(#glow)"` を**絶対に適用しない**。一部のSVGレンダリング環境（ブラウザプレビュー、GitHub等）で矢印が完全に消えてしまう不具合が確認されている。glowはアクターカード等の装飾に限定し、矢印には使用しないこと
7. **矢印の方向 = アクションの論理的意味**: 矢印の向きは必ず論理的なアクションと一致させること。リクエスト/ダウンロード要求 = 要求元→サーバー方向、レスポンス/データ返却 = サーバー→要求元方向。「ファイルをダウンロードする」矢印はダウンロード要求者からサーバーへ向かうべきであり、逆向きにしない
8. **L字パスによるテキスト被り回避**: 矢印が他の要素（ボックス、テキスト、他の矢印）を横切るリスクがある場合、斜め線や曲線ではなく **L字パス** を使用する。`M x1 y1 L x2 y1 L x2 y2`（水平→垂直）のように直角に折り曲げ、`stroke-linejoin="round"` で角を丸くすることで、テキストやラベルとの重なりを確実に回避できる
9. **結節点の等間隔配置**: 1つのアクターの辺（底辺・側辺等）から複数の矢印が出る場合、**アクター中心を軸に等間隔で配置**すること。例: 中心x=600の底辺から3本の矢印 → x=560, x=600, x=640（間隔40px）。偏った配置は不安定な印象を与える
10. **キャンバス幅の最大活用**: アクター間の距離を十分に確保し、箱やテキストが中央に密集しないようにする。viewBox幅を最大限活かして左右の余白と要素間スペースを大きく取ること。URLラベルや矢印番号バッジが窮屈にならないよう、アクター間は最低200px以上の間隔を確保する
11. **下書き要素の残存禁止**: ガイド線、調整用点線、座標確認用マーカーなど、最終SVGに不要な要素を残さないこと。意味のある要素（凡例線、区切り線等）以外の補助的な線は削除する
12. **防御テクニックカードのグリッド配置（対策図）**: 対策図に配置する防御テクニックカード（詳細説明付きのカード群）は、すべて**同一サイズ・グリッド配置**にすること。サイズや位置がバラバラだと図全体の統一感が崩れる。具体的には:
    - 全カードを同じ幅・高さ（標準: 250×120px）で統一
    - 2枚なら横並び、3〜4枚なら2×2グリッド、5〜6枚なら2×3または3×2グリッドで配置
    - カード内部レイアウトも統一: 番号バッジ（左上）→ タイトル → 説明 → 区切り線 → コード例/詳細
    - グリッドの上に「防御テクニック」セクションヘッダー（pill型バッジ + 区切り線）を置き、攻撃フロー部分と視覚的に分離する
    - グリッドの列間隔は80〜100px、行間隔は30〜50pxを確保
13. **対策図のコンテンツ中央揃え**: 対策図では右サイドバー（x=785〜1080）を除いたコンテンツ領域（x=0〜770）の中心 **x=385** を基準に、タイトル・セクションヘッダー・カードグリッド・凡例を中央揃えすること。タイトルは `x=385, text-anchor="middle"`、カードグリッドは `CX=215/555`、凡例は `translate(205, 800)` に配置する
14. **対策図の凡例と下部余白**: 凡例は防御テクニックカードの下に十分な余白（12px以上）を確保して配置すること。カード下端（y≈788）と凡例上端（y=800）が重ならないよう、viewBox高さは `840` 以上に設定する
