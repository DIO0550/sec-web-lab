...
**サイズの柔軟な調整**: 以下のテンプレートは標準サイズ（112x125px）だが、矢印の本数が多く間隔が窮屈になる場合はカードを拡大してよい。具体的には:
- カードの `rect` の `height` を 155px 程度まで拡大（`y` 開始位置も `-35` 等に調整）
- アイコングループに `scale(1.15)` 等を追加して拡大（例: `transform="translate(0, 12) scale(1.15)"`）
- テキストの `y` 座標をカード下部に合わせて調整（例: y=72→85, y=86→99）
- viewBox の高さも拡大分に応じて増やす

矢印間の最低間隔（42px以上）を確保することを優先し、カードサイズはそのための手段として調整する。
