...
対策図: viewBox="0 0 1100 840"  (防御カード + 凡例の余白確保)
```
...
対策図では右サイドバー（x=785〜1080）を除いたコンテンツ領域（x=0〜770）の中心 **x=385** を基準に、タイトル・セクションヘッダー・カードグリッド・凡例を中央揃えすること。

```
コンテンツ領域: 0 ─────── 385 ─────── 770 │ サイドバー │
                        ↑ 中心
タイトル:    text-anchor="middle" x=385
セクションヘッダー: text x=385, pill rect x=315
カードグリッド: Column1 CX=215, Column2 CX=555 (中心 = 385)
凡例: translate(205, 800) → 幅360px, 中心=385
```
...
拡大サイズ (矢印が3本以上でY間隔が窮屈な場合):
  幅: 112px  高さ: 155px  角丸: 12px
  アイコン: scale(1.15) で拡大
  テキスト: y座標を +13px 下にずらす (72→85, 86→99)

中心X位置: 115 (左), 380-400 (中央), 645 (右)  ※攻撃図基準
中心Y位置: 250 (共通)
```

矢印間の最低間隔（42px）が確保できない場合はカードを拡大サイズにする。viewBoxの高さも拡大分だけ増やすこと。
...
防御テクニックカード (グリッド配置):
  統一サイズ: 250x120px  角丸: 8px
  グリッド列間隔: 80〜100px
  グリッド行間隔: 30〜50px
  配置例 (2×2):
    Column1 center: X=215  Column2 center: X=555
    Row1 center: Y=535     Row2 center: Y=690
  セクションヘッダー: グリッド上部に「防御テクニック」pill型バッジ (x=315) + 区切り線

カード内部レイアウト (統一):
  番号バッジ: x=-108, y=-12, 22x22px, rx=5
  タイトル:   x=-78, y=4, font-size=11, font-weight=700, defense-light
  説明:       x=-78, y=22, font-size=9, defense-pale
  区切り線:   x=-108〜108, y=34
  コード例:   x=0 (center), y=54〜88, font-size=9, monospace
```
